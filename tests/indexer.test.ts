...
test("indexes sessions into sqlite and exposes timeline, tools, subagents, and raw events", async () => {
  ...
  await writeFile(join(dayDir, "rollout-2026-06-14T00-01-00-child-1.jsonl"), `${JSON.stringify({ timestamp: "2026-06-14T00:01:00.000Z", type: "session_meta", payload: { id: "child-1", cwd: "/work", thread_source: "subagent", source: { subagent: { thread_spawn: { parent_thread_id: "parent-1", agent_nickname: "Curie", agent_role: "worker", depth: 1 } } } } })}\n`);
